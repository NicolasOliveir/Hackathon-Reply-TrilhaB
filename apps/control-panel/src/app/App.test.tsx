...
describe('painel responsivo e auditoria',()=>{
 ...
 it('filtra e limpa auditoria',async()=>{render(<App/>);submit();await waitFor(()=>expect(screen.getByText('Briefing recebido')).toBeInTheDocument());fireEvent.click(screen.getByRole('button',{name:'Sistema'}));expect(screen.getAllByText(/Execu/).length).toBeGreaterThan(0);fireEvent.click(screen.getByRole('button',{name:'Limpar'}));expect(screen.getByText(/Nenhum evento/)).toBeInTheDocument()});
 it('distingue erro de transporte e permite reconectar',()=>{render(<App/>);fireEvent.click(screen.getByRole('button',{name:/simular erro/i}));expect(screen.getByRole('alert')).toHaveTextContent('Falha de transporte');expect(screen.getByRole('button',{name:/tentar/i})).toBeInTheDocument()});
});
